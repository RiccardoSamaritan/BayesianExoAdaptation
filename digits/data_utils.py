...
digits/download_datasets.sh and applies the shared preprocessing every
domain needs before they can be fed to the same model:
...
This module only depends on numpy/torch/torchvision/PIL and the .npz caches
in digits/data/ -- it does not import anything from src/, and nothing in
src/ imports from here (per the project's convention that this experiment's
own code stays confined to digits/, importing FROM src/bayesian.py,
src/im_adapt.py, src/calibration.py where needed by notebooks, not the
other way around).
"""
...
def _load_raw(domain: str, split: str) -> tuple:
    ...
    if not npz_path.exists():
        raise FileNotFoundError(f"{npz_path} not found -- run digits/download_datasets.sh first")
...
if __name__ == "__main__":
    print("Image counts per domain/split (digits/data/*.npz):")
    print("-" * 60)
    ...
    print("\nExample: mnist as source, all domains loaded at 32x32 grayscale, "
          "normalized to mnist-train statistics:")
    mean, std = compute_source_stats("mnist")
    print(f"  mnist-train mean={mean:.4f} std={std:.4f}")
    for domain in DOMAINS:
        X, y = load_domain(domain, "test", mean, std)
        print(f"  {domain:6s} test:  X{tuple(X.shape)} y{tuple(y.shape)}  "
              f"(post-normalization mean={X.mean().item():+.4f} std={X.std().item():.4f})")
